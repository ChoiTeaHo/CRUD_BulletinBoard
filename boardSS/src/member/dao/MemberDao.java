...
public class MemberDao {
	public Member selectById(java.sql.Connection conn, String id) throws SQLException {
		...
		try {
			...
			if (rs.next()) {
				member = new Member(rs.getString("memberid"), rs.getString("name"), rs.getString("password"),
						toDate(rs.getTimestamp("regdate"))); // 변환
			}
			return member; // select한 결과를 각 Memeber객체에 생성자로 저장하고 리턴한다.
		} finally {
			JdbcUtil.close(rs); // 편하게 닫기
			JdbcUtil.close(pstmt);
		}
	}

	private Date toDate(Timestamp date) { // date값이 없으면 null / 값이있으면 Date객체생성
